...
class Task {
     ...
     * @param string $sort_param Paramètre de tri (ex: 'due_date ASC', 'created_at DESC').
    ...
    public function read($user_id, $filters = [], $sort_param = 'created_at DESC') {
        ...
        // Gère le tri de manière sécurisée en utilisant une liste blanche
        $allowed_sort_params = [
            'due_date ASC',
            'due_date DESC',
            'created_at DESC',
        ...
        // Utilise le paramètre de tri s'il est dans la liste blanche, sinon utilise la valeur par défaut
        $order_by = in_array($sort_param, $allowed_sort_params) ? $sort_param : 'created_at DESC';
